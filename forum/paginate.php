    ...
    */
    ...
    if ($startRow == 0) {
        ...
        <tr valign="top">
            ...
            if ($qryMain["intAllowChat"] && $_SESSION["MemberID"]) {
                ...
                if ($qryMain["intAllowChat"] == 1 || ($qryMain["intAllowChat"] == 2 && $_SESSION["MemberID"]) || ($qryMain["intAllowChat"] == 3 && $_SESSION["AccessLevel"] >= 90)) {
                    ...
                    if (strlen($qryMain["strAIM"])) {
                        print "<a href=\"aim:goim?screenname=" . trim($qryMain["strAIM"]) . "&message=G-Net+member+chat\"><img src=\"/forum/images/aim.gif\" width=\"26\" height=\"20\" alt=\"Send AIM\" border=\"0\"></a>";
                    }
                    
                    // check ICQ
                    if (strlen($qryMain["strICQ"])) {
                        print "<a href=\"http://wwp.icq.com/scripts/search.dll?to=" . trim($qryMain["strICQ"]) . "\" target=\"_new\"><img src=\"/forum/images/icq.gif\" width=\"18\" height=\"20\" alt=\"Send ICQ\" border=\"0\"></a>";
                    }
                    
                    // check MSN Messenger
                    if (strlen($qryMain["strMSN"])) {
                        print "<img src=\"/forum/images/msn.gif\" width=\"21\" height=\"20\" alt=\"" . trim($qryMain["strMSN"]) . "\" border=\"0\">";
                    }
                    
                    // check Yahoo! Messenger
                    if (strlen($qryMain["strYahoo"])) {
                        print "<a href=\"http://edit.yahoo.com/config/send_webmesg?.target=" . trim($qryMain["strYahoo"]) . "&.src=pg\" target=\"_new\"><img src=\"/forum/images/yahoo.gif\" width=\"24\" height=\"20\" alt=\"Send Yahoo! Message\" border=\"0\"></a>";
                    }
                }
                print "<br>";
            }
            ...
            if ($_SESSION["MemberID"] == $qryMain["intMemID"] || $_SESSION["AccessLevel"] >= 90) {
                ?>
                <br>
                &raquo; <a href="/forum/edit/post.php?id=<?php print $qryMain["topicID"]; ?>"><b>Edit Topic</b></a><br>
        ...
        if ($_SESSION["MemberID"] && $qryMain["ID"] != $_SESSION["MemberID"]) {
            ...
            if (!$qryMain["intBanned"]) {
                ...
                &raquo; <a href="/members/buddy.php?id=<?php print $qryMain["intMemID"]; ?>&return=/forum/view.php?<?php print $_SERVER["QUERY_STRING"]; ?>"><b>Add to "Buddy List"</b></a><br>
                <?php
            }
            }
            ...
            <tr>
                ...
                if ($_SESSION["MemberID"] && $qryMain["bitReply"]) {
                    print "
                    &raquo; <a href=\"/forum/reply/index.php?thread=" . $_GET["thread"] . "&quote=1\"><b>Reply With Quote</b></a><br>";
                }
    ...
    if ($qryReplies->numRows()) {
        ...
        while ($qryRow = $qryReplies->fetchRow(DB_FETCHMODE_ASSOC)) {
            print "<!-- Row #$rowCount start ($startRow) - end ($endRow) -->\n";
            ...
            if ($rowCount >= $startRow && $rowCount <= $endRow) {
                ...
                <tr>
                    ...
                    if ($qryRow["intAllowChat"] && $_SESSION["MemberID"]) {
                        ...
                        if ($qryRow["intAllowChat"] == 1 || ($qryRow["intAllowChat"] == 2 && $_SESSION["MemberID"]) || ($qryRow["intAllowChat"] == 3 && $_SESSION["AccessLevel"] >= 90)) {
                            ...
                            if (strlen($qryRow["strAIM"])) {
                                print "<a href=\"aim:goim?screenname=" . trim($qryRow["strAIM"]) . "&message=G-Net+member+chat\"><img src=\"/forum/images/aim.gif\" width=\"26\" height=\"20\" alt=\"Send AIM\" border=\"0\"></a>";
                            }
                            // check ICQ
                            if (strlen($qryRow["strICQ"])) {
                                print "<a href=\"http://wwp.icq.com/scripts/search.dll?to=" . trim($qryRow["strICQ"]) . "\" target=\"_new\"><img src=\"/forum/images/icq.gif\" width=\"18\" height=\"20\" alt=\"Send ICQ\" border=\"0\"></a>";
                            }
                            // check MSN Messenger
                            if (strlen($qryRow["strMSN"])) {
                                print "<img src=\"/forum/images/msn.gif\" width=\"21\" height=\"20\" alt=\"" . trim($qryRow["strMSN"]) . "\" border=\"0\">";
                            }
                            // check Yahoo! Messenger
                            if (strlen($qryRow["strYahoo"])) {
                                print "<a href=\"http://edit.yahoo.com/config/send_webmesg?.target=" . trim($qryRow["strYahoo"]) . "&.src=pg\" target=\"_new\"><img src=\"/forum/images/yahoo.gif\" width=\"24\" height=\"20\" alt=\"Send Yahoo! Message\" border=\"0\"></a>";
                            }
                        }
                        print "<br>";
                    }
                    ...
                    if ($_SESSION["MemberID"] == $qryRow["intMemID"] || $_SESSION["AccessLevel"] >= 90) {
                        ?>
                        <br>
                        &raquo; <a href="/forum/edit/reply.php?id=<?php print $qryRow["replyID"]; ?>"><b>Edit Reply</b></a><br>
                    ...
                    if ($_SESSION["MemberID"] && $qryRow["intMemID"] != $_SESSION["MemberID"]) {
                        ...
                        if (!$qryRow["intBanned"]) {
                            ...
                            &raquo; <a href=\"/members/buddy.php?id=" . $qryRow["intMemID"] . "&return=/forum/view.php?" . $_SERVER["QUERY_STRING"] . "\"><b>Add to \"Buddy List\"</b></a><br>\n";
                        }
                    }
                    ...
                    <tr>
                        ...
                        if ($_SESSION["MemberID"] && $qryMain["bitReply"]) {
                            print "&raquo; <a href=\"/forum/reply/index.php?thread=" . $_GET["thread"] . "&reply=" . $qryRow["replyID"] . "&quote=1\"><b>Reply With Quote</b></a><br>";
                        }
        ...
        if ($qryReplies->numRows() > $intDisplayNum) {
            ...
            $strURL = "/forum/view.php?forum=". $_GET["forum"] . "&thread=" . $_GET["thread"] . "&display=$intDisplayNum&sort=$sortOrder&";
