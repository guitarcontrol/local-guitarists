...
<?php
    ...
    */
    ...
    // see if we need to display the first post
    if ($startRow == 0) {
        ?>
        <tr>
        ...
        <tr valign="top">
            ...
            if ($qryMain["intAllowChat"] && $_SESSION["MemberID"]) {
                ...
                if ($qryMain["intAllowChat"] == 1 || ($qryMain["intAllowChat"] == 2 && $_SESSION["MemberID"]) || ($qryMain["intAllowChat"] == 3 && $_SESSION["AccessLevel"] >= 90)) {
                    ...
                    if (strlen($qryMain["strAIM"])) {
                        print "<a href=\"aim:goim?screenname=" . trim($qryMain["strAIM"]) . "&message=G-Net+member+chat\" title=\"Send AIM to " . trim($qryMain["strAIM"]) . "\"><img src=\"/forum/images/aim.gif\" width=\"26\" height=\"20\" alt=\"Send AIM to " . trim($qryMain["strAIM"]) . "\" border=\"0\"></a>";
                    }
                    
                    // check ICQ
                    if (strlen($qryMain["strICQ"])) {
                        print "<a href=\"http://wwp.icq.com/scripts/search.dll?to=" . trim($qryMain["strICQ"]) . "\" target=\"_new\" title=\"Send ICQ to " . trim($qryMain["strICQ"]) . "\"><img src=\"/forum/images/icq.gif\" width=\"18\" height=\"20\" alt=\"Send ICQ to " . trim($qryMain["strICQ"]) . "\" border=\"0\"></a>";
                    }
                    
                    // check MSN Messenger
                    if (strlen($qryMain["strMSN"])) {
                        print "<img src=\"/forum/images/msn.gif\" width=\"21\" height=\"20\" alt=\"Send IM to " . trim($qryMain["strMSN"]) . "\" title=\"Send IM to " . trim($qryMain["strMSN"]) . "\" border=\"0\">";
                    }
                    
                    // check Yahoo! Messenger
                    if (strlen($qryMain["strYahoo"])) {
                        print "<a href=\"http://edit.yahoo.com/config/send_webmesg?.target=" . trim($qryMain["strYahoo"]) . "&.src=pg\" target=\"_new\" title=\"Send Yahoo! Message to " . trim($qryMain["strYahoo"]) . "\"><img src=\"/forum/images/yahoo.gif\" width=\"24\" height=\"20\" alt=\"Send Yahoo! Message to " . trim($qryMain["strYahoo"]) . "\" border=\"0\"></a>";
                    }
                }
                print "<br>";
            }
            ...
            if ($_SESSION["MemberID"] == $qryMain["intMemID"] || $_SESSION["AccessLevel"] >= 90) {
                ?>
                <br>
                &raquo; <a href="/forum/edit/post_bb.php?id=<?php print $qryMain["topicID"]; ?>"><b>Edit Topic</b></a><br>
                &raquo; <a href="javascript:plainWin('/forum/tools/delete.php?topic=<?php print $_GET["thread"]; ?>','100','20')" onClick="return confirm('Are you sure you want to delete this entire thread?');"><b>Delete Thread</b></a><br>
                <?php
            }
            ...
            if ($qryMain["bitReply"]) {
                ?>
                &raquo; <a href="javascript:plainWin('/forum/tools/close.php?forum=<?php print $_GET["forum"]; ?>&topic=<?php print $_GET["thread"]; ?>&active=0','150','20')"><b>Close Topic</b></a><br>
                <?php
            } else {
                ?>
                &raquo; <a href="javascript:plainWin('/forum/tools/close.php?forum=<?php print $_GET["forum"]; ?>&topic=<?php print $_GET["thread"]; ?>&active=1','150','20')"><b>Open Topic</b></a><br>
                <?php
            }
            ?>
            &raquo; <a href="javascript:plainWin('/forum/tools/move.php?topic=<?php print $_GET["thread"]; ?>','250','100')"><b>Move Thread</b></a><br>
            <?php
        }
        ...
        if ($_SESSION["MemberID"] && $qryMain["ID"] != $_SESSION["MemberID"]) {
            ...
            if (count($arrBlocked)) {
                ...
                if (in_array($qryMain["intMemID"], $arrBlocked)) {
                    print "&raquo; <a href=\"javascript:plainWin('/forum/tools/block.php?id=" . $qryMain["intMemID"] . "&forum=" . $_GET["forum"] . "&topic=" . $_GET["thread"] . "&ban=0','100','20')\"><b>Unblock Member</b></a><br>";
                } else {
                    print "&raquo; <a href=\"javascript:plainWin('/forum/tools/block.php?id=" . $qryMain["intMemID"] . "&forum=" . $_GET["forum"] . "&topic=" . $_GET["thread"] . "&ban=1','100','20')\" onClick=\"return confirm('Are you sure you want to block all posts by \'" . addslashes($qryMain["strUsername"]) . "\'?')\"><b>Block Member</b></a><br>";
                }
            } else {
                print "
                &raquo; <a href=\"javascript:plainWin('/forum/tools/block.php?id=" . $qryMain["intMemID"] . "&forum=" . $_GET["forum"] . "&topic=" . $_GET["thread"] . "&ban=1','100','20')\" onClick=\"return confirm('Are you sure you want to block all posts by \'" . addslashes($qryMain["strUsername"]) . "\'?')\"><b>Block Member</b></a><br>";
            }
            
            // see if they're not banned
            if (!$qryMain["intBanned"]) {
                ?>
                &raquo; <a href="/forum/msgs/post/index_bb.php?user=<?php print $qryMain["intMemID"]; ?>"><b>Send Private Message</b></a><br>
                &raquo; <a href="/members/buddy.php?id=<?php print $qryMain["intMemID"]; ?>&return=/forum/view_bb.php?<?php print $_SERVER["QUERY_STRING"]; ?>"><b>Add to "Buddy List"</b></a><br>
                <?php
            }
            }
            ...
            <tr>
                ...
                if ($_SESSION["MemberID"] && $qryMain["bitReply"]) {
                    print "
                    &raquo; <a href=\"/forum/reply/index_bb.php?thread=" . $_GET["thread"] . "&quote=1\"><b>Reply With Quote</b></a><br>";
                }
                
                // see if it can be reported
                if ($_SESSION["MemberID"]) {
                    print "
                    &raquo; <a href=\"/forum/report.php?thread=" . $_GET["thread"] . "&forum=" . $_GET["forum"] . "&topic=" . $_GET["thread"] . "\" onClick=\"return confirm('Are you sure you want to report this thread?')\"><b>Report To A Moderator</b></a>";
                }
                ?>
                </td>
            </tr>
    ...
    if ($qryReplies->numRows()) {
        ...
        // see if we need to update our endRow
        if ($endRow > $qryReplies->numRows()) {
            $endRow = $qryReplies->numRows();
        }
        
        // set our half page cutoff to display our inline Google ad
        $middleRow = $endRow - ceil($startRow / 2);
        
        // output our replies, based on the page chosen
        while ($qryRow = $qryReplies->fetchRow(DB_FETCHMODE_ASSOC)) {
            // see if it should be displayed or not
            if ($rowCount >= $startRow && $rowCount <= $endRow) {
                ...
                <tr>
                    ...
                    if ($qryRow["intAllowChat"] && $_SESSION["MemberID"]) {
                        ...
                        if ($qryRow["intAllowChat"] == 1 || ($qryRow["intAllowChat"] == 2 && $_SESSION["MemberID"]) || ($qryRow["intAllowChat"] == 3 && $_SESSION["AccessLevel"] >= 90)) {
                            ...
                            if (strlen($qryRow["strAIM"])) {
                                print "<a href=\"aim:goim?screenname=" . trim($qryRow["strAIM"]) . "&message=G-Net+member+chat\" title=\"Send AIM to " . trim($qryRow["strAIM"]) . "\"><img src=\"/forum/images/aim.gif\" width=\"26\" height=\"20\" alt=\"Send AIM to " . trim($qryRow["strAIM"]) . "\" border=\"0\"></a>";
                            }
                            // check ICQ
                            if (strlen($qryRow["strICQ"])) {
                                print "<a href=\"http://wwp.icq.com/scripts/search.dll?to=" . trim($qryRow["strICQ"]) . "\" target=\"_new\" title=\"Send ICQ to " . trim($qryRow["strICQ"]) . "\"><img src=\"/forum/images/icq.gif\" width=\"18\" height=\"20\" alt=\"Send ICQ to " . trim($qryRow["strICQ"]) . "\" border=\"0\"></a>";
                            }
                            // check MSN Messenger
                            if (strlen($qryRow["strMSN"])) {
                                print "<img src=\"/forum/images/msn.gif\" width=\"21\" height=\"20\" alt=\"Send IM to " . trim($qryRow["strMSN"]) . "\" title=\"Send IM to " . trim($qryRow["strMSN"]) . "\" border=\"0\">";
                            }
                            // check Yahoo! Messenger
                            if (strlen($qryRow["strYahoo"])) {
                                print "<a href=\"http://edit.yahoo.com/config/send_webmesg?.target=" . trim($qryRow["strYahoo"]) . "&.src=pg\" target=\"_new\" title=\"Send Yahoo! Message to " . trim($qryRow["strYahoo"]) . "\"><img src=\"/forum/images/yahoo.gif\" width=\"24\" height=\"20\" alt=\"Send Yahoo! Message to " . trim($qryRow["strYahoo"]) . "\" border=\"0\"></a>";
                            }
                        }
                        print "<br>";
                    }
                    ...
                    if ($_SESSION["MemberID"] == $qryRow["intMemID"] || $_SESSION["AccessLevel"] >= 90) {
                        ?>
                        <br>
                        &raquo; <a href="/forum/edit/reply_bb.php?id=<?php print $qryRow["replyID"]; ?>"><b>Edit Reply</b></a><br>
                        &raquo; <a href="javascript:plainWin('/forum/tools/delete.php?reply=<?php print $qryRow["replyID"]; ?>','100','20')" onclick="return confirm('Are you sure you want to delete this reply?');"><b>Delete Reply</b></a><br>
                        <?php
                    }
                    ...
                    if ($_SESSION["MemberID"] && $qryRow["intMemID"] != $_SESSION["MemberID"]) {
                        ...
                        if (count($arrBlocked)) {
                            ...
                            if (in_array($qryRow["intMemID"], $arrBlocked)) {
                                print "&raquo; <a href=\"javascript:plainWin('/forum/tools/block.php?id=" . $qryRow["intMemID"] . "&forum=" . $_GET["forum"] . "&topic=" . $_GET["thread"] . "&ban=0','100','20')\"><b>Unblock Member</b></a><br>";
                            } else {
                                print "&raquo; <a href=\"javascript:plainWin('/forum/tools/block.php?id=" . $qryRow["intMemID"] . "&forum=" . $_GET["forum"] . "&topic=" . $_GET["thread"] . "&ban=1','100','20')\" onClick=\"return confirm('Are you sure you want to block all posts by \'" . addslashes($qryRow["strUsername"]) . "\'?')\"><b>Block Member</b></a><br>";
                            }
                        } else {
                            print "
                            &raquo; <a href=\"javascript:plainWin('/forum/tools/block.php?id=" . $qryRow["intMemID"] . "&forum=" . $_GET["forum"] . "&topic=" . $_GET["thread"] . "&ban=1','100','20')\" onClick=\"return confirm('Are you sure you want to block all posts by \'" . addslashes($qryRow["strUsername"]) . "\'?')\"><b>Block Member</b></a><br>";
                        }
                        if (!$qryRow["intBanned"]) {
                            print "
                            &raquo; <a href=\"/forum/msgs/post/index_bb.php?user=" . $qryRow["intMemID"] . "\"><b>Send Private Message</b></a><br>
                            &raquo; <a href=\"/members/buddy.php?id=" . $qryRow["intMemID"] . "&return=/forum/view_bb.php?" . $_SERVER["QUERY_STRING"] . "\"><b>Add to \"Buddy List\"</b></a><br>\n";
                        }
                    }
                    ...
                    <td>
                    <?php
                    print "<!-- $rowCount - $startRow - $endRow -->\n";
                    // if this is the first row, display our Google ads
                    if (!$_SESSION["HideAds"] && (($startRow && $rowCount == $startRow) || (!$startRow && $rowCount == $endRow) || ($startRow && $rowCount == $endRow - 1))) {
                        ?>
                        <!--- start our Google ad --->
                        <table width="300" cellspacing="4" cellpadding="4" border="0" align="right">
                        <tr>
                    ...
                    <tr>
                        ...
                        if ($_SESSION["MemberID"] && $qryMain["bitReply"]) {
                            print "&raquo; <a href=\"/forum/reply/index_bb.php?thread=" . $_GET["thread"] . "&reply=" . $qryRow["replyID"] . "&quote=1\"><b>Reply With Quote</b></a><br>";
                        }
                        // create a link to report the thread
                        if ($_SESSION["MemberID"]) {
                            print "&raquo; <a href=\"/forum/report.php?thread=" . $_GET["thread"] . "&forum=" . $_GET["forum"] . "&reply=" . $qryRow["replyID"] . "\" onClick=\"return confirm('Are you sure you want to report this thread?')\"><b>Report To A Moderator</b></a>";
                        }
                        ?>
                        </td>
                    </tr>
        ...
        if ($qryReplies->numRows() > $intDisplayNum) {
            ...
            $strURL = "/forum/view_bb.php?forum=". $_GET["forum"] . "&thread=" . $_GET["thread"] . "&display=$intDisplayNum&sort=$sortOrder&";
